 ...
 */
class ThemeManager(private val dataDir: String = "arcade-data/themes") {
    ...
    fun getCustomThemes(): List<Theme> {
        ...
        return customDir.listFiles { file -> file.extension == "json" }
            ?.mapNotNull { file ->
                try {
    ...
    fun getAllThemes(): List<Theme> {
        return getDefaultThemes() + getCustomThemes()
    }
     ...
     */
    suspend fun addThemeFromUrl(url: String): Result<Theme> = withContext(Dispatchers.IO) {
        try {
            ...
            val theme = if (url.endsWith(".yaml") || url.endsWith(".yml")) {
     ...
     */
    suspend fun updateTheme(theme: Theme, url: String): Result<Theme> {
        return addThemeFromUrl(url)
    }
